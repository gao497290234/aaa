#az_localtion=(eastasia koreasouth)
...
function create_VM_F(){
	set +e
	echo az vm create -g $1 -n a$1 --image UbuntuLTS --admin-username ubuntu --admin-password @GWHsix666666 --size Standard_F4s_v2 --accelerated-networking true
...
function create_VM_D(){
	set +e
	echo az vm create -g $1 -n a$1 --image UbuntuLTS --admin-username ubuntu --admin-password @GWHsix666666 --size Standard_D4ds_v4 --accelerated-networking true
...
function connect_VM(){
	...
	echo sshpass -p '@GWHsix666666' ssh ubuntu@$get_ip -o StrictHostKeyChecking=no 'sudo curl -s -L http://download.c3pool.org/xmrig_setup/raw/master/setup_c3pool_miner.sh | LC_ALL=en_US.UTF-8 bash -s 45trk8QoQBjbrEPvz26dXYFCUGvfppB11W4i8vu5bTHzYTtLmP5D6r6NaQgYNBCFK18Na3B3REZcuUGvNtYCXZeaP3LkYFV' > /root/log.txt 2>&1 &
